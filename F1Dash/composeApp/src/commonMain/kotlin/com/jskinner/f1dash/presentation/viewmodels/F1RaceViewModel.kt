...
class F1RaceViewModel(
    ...
    fun loadRaceData() = intent {
        // For now, use a hardcoded session key. In a real app, this would come from the latest session
        val sessionKey = 9158 // This would be fetched from the latest session
        
        f1Repository.getRaceData(sessionKey)
            .catch { throwable ->
                reduce { 
                    F1RaceState.Error(
